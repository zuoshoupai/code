...
class OutPut(object):
	...
	def output_sql(self):
		...
		for data in self.datas: 
			...
			sql = "insert into ask_articles (user_id,url,logo,title,summary,content,created_at,status,category_id) values ('%s','%s','%s','%s','%s','%s','%s','%s','%s')" % ('1',url,img,title,des,body,time_date,'1','8')
			try: 
				cursor.execute(sql)
			except Exception, e:
				print url
	...
	def output_sql_single(self,data):
		...
		url   = data['url']
		img   = str(data['img']).replace(root,'')
		body  = MySQLdb.escape_string(str(data['body']).encode('utf-8'))
		title = data['title'] 
		tag   = data['tag']
		des   = MySQLdb.escape_string(str(data['des']))
		#time  = data['date'] 
		...
		data = cursor.fetchone()
		if(data[0]==0): 
			sql = "insert into ask_articles (user_id,url,logo,title,summary,content,created_at,status,category_id,source) values ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')" % ('1',url,img,title,des,body,time_date,'1','8','网易资讯')
			try: 
				cursor.execute(sql)
			except Exception, e:
				print url
